...
export default function PostEditor({ post }: Props) {
  ...
  const handleCoverUrlSubmit = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Cover image URL upload failed");
    } finally {
      setCoverUploading(false);
    }
  }, [coverUrlInput]);
  ...
  const handleSubmit = async (submitStatus?: SerializedPost["status"]) => {
    ...
    const payload: Record<string, unknown> = {
      ...
      scheduledFor:
        effectiveStatus === "scheduled" && scheduledFor ? scheduledFor : undefined,
      publishDate:
        effectiveStatus === "published" && publishDate ? publishDate : undefined,
      sendNewsletter,
    };
